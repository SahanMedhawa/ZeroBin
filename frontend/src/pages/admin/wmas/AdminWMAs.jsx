...
function AdminWMAs() {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`Total WMAs: ${calculateTotalWMAs()}`, 14, 58);
    doc.text(`Total Collectors: ${getTotalCollectors()}`, 14, 66);
    doc.text(`Top Performer: ${findWMAWithHighestCollectors()}`, 14, 74);
    ...
    const tableData = filteredWMAs.map((wma) => [
      ...
      columnStyles: {
        0: { cellWidth: 25 }, // WMA Name
        1: { cellWidth: 40 }, // Email - reduced slightly
        2: { cellWidth: 30 }, // Auth Number - increased to prevent wrapping
        3: { cellWidth: 15 }, // Collectors - narrow for numbers
        4: { cellWidth: 35 }, // Address - reduced slightly
        5: { cellWidth: 30 }  // Contact - increased to prevent wrapping
      },
