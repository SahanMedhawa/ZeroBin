...
function AdminUsers() {
  ...
  const downloadPDF = () => {
    ...
    doc.text(`Total Users: ${calculateTotalUsers()}`, 14, 58);
    doc.text(`Areas Covered: ${getUniqueAreas().length}`, 14, 66);
    doc.text(`Active Users: ${users.filter(u => u.area?.name).length}`, 14, 74);
    ...
    const tableData = filteredUsers
      ...
      columnStyles: {
        ...
        1: { cellWidth: 50 }, // Email - wider for long emails
        2: { cellWidth: 25 }, // Area
        3: { cellWidth: 40 }, // Address - wider for long addresses
