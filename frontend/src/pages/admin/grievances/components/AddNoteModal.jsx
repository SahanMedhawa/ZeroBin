...
const AddNoteModal = ({ 
  ...
  const [noteData, setNoteData] = useState({ content: '', noteType: 'Update' });
  const textareaRef = useRef(null);

  // Reset form when modal opens/closes
  useEffect(() => {
    if (isOpen) {
      setNoteData({ content: '', noteType: 'Update' });
    }
  }, [isOpen]);
  ...
  const handleContentChange = useCallback((e) => {
    ...
    setNoteData(prevData => ({
      noteType: prevData.noteType,
      content: newValue
    }));
  }, []);

  const handleNoteTypeChange = useCallback((e) => {
    const newValue = e.target.value;
    setNoteData(prevData => ({
      noteType: newValue,
  ...
  }, [noteData, onAddNote, handleClose]);
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                required
              />
              <p className="text-xs text-gray-500 mt-1">{noteData.content.length}/1000 characters</p>
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-3 mt-6">
            ...
            <button
              type="submit"
              disabled={addingNote || !noteData.content.trim()}
